    ...
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
      {
        ...
        <label htmlFor='user-img'>
          <div className='inline-block relative cursor-pointer'>
            <img className='w-36 rounded opacity-75' src={userImage ? URL.createObjectURL(userImage) : userData.image} alt="" />
            <img className='w-10 absolute bottom-[50%] right-[50%]' src={userImage ? "" : assets.upload_icon} alt="" />
          </div>
          <input onChange={(e) => setUserImage(e.target.files[0])} type="file" id='user-img' hidden />
        </label>
          :
      ...
      <div>
        ...
        <div className="grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700">
          <p className='font-medium'>Email Address</p>
          {
            isEdit
              ?
            <input className='bg-gray-50 font-medium max-w-52' onChange={(e) => setUserData(prev => ({ ...prev, email: e.target.value }))} value={userData.email} type="email" />
              :
      ...
      <div className="mt-10">
        {
          isEdit
            ?
          <button className="border border-primary px-8 py-2 rounded-full hover:bg-primary hover:text-white transition-all" onClick={updateProfile}>Save Information</button>
            :
